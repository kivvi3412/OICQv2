...
void *server_response(char *input_json, client_info *client) {
    ...
    if (strcmp(cmd->valuestring, "login") == 0) {   // login
        ...
        return login_from_json(input_json);
        ...
        return register_from_json(input_json);
    } else if (strcmp(cmd->valuestring, "get_history") == 0) {  // get_history
        cJSON *token = cJSON_GetObjectItem(json_obj, "parm1");
        if (!token) {
        ...
        // 添加消息到数据库
        char *username = check_token(token->valuestring);
        if (!username) {
        ...
        // 群发消息
        broadcast_message_func(input_json, client->socket_fd);
        return send_status_message_to_client(json_server);
    } else {
        printf("Invalid cmd\n");
        return input_json;
    }
